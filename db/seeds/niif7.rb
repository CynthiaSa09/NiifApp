...
(iii) una indicación de la frecuencia con que se discontinúan y reinician las relaciones de cobertura como parte del proceso de la entidad en relación con las relaciones de cobertura.', number: '23c', order: 62)
niif.contents.create!(text:'Una entidad revelará por categoría de riesgo una descripción de los orígenes de ineficacia de cobertura que se espera que afecten a la relación de cobertura durante su duración.', number: '23d', order: 63)
niif.contents.create!(text:'Si emergen otros orígenes de ineficacia de cobertura en una relación de cobertura, una entidad revelará dichos orígenes por categoría de riesgo y explicarán la ineficacia de cobertura resultante.', number: '23e', order: 64)
niif.contents.create!(text:'Para coberturas de flujos de efectivo, una entidad revelará una descripción de las transacciones previstas para las cuales se ha utilizado la contabilidad de coberturas en el periodo anterior, pero que no se espera que ocurran más.', number: '23f', order: 64)

niif.contents.create!(text: 'Los efectos de la contabilidad de coberturas sobre la situación financiera y el rendimiento', label: 3, order: 65)
niif.contents.create!(text:'[Eliminado]', number: '24', order: 66)
niif.contents.create!(text:'Una entidad revelará, en forma de tabla, los importes siguientes relacionados con partidas designadas como instrumentos de cobertura de forma separada por categoría de riesgo para cada tipo de cobertura (cobertura del valor razonable, cobertura de flujos de efectivo o cobertura de una inversión neta en un negocio en el extranjero):
...
(d) los importes nominales (incluyendo cantidades tales como toneladas o metros cúbicos) de los instrumentos de cobertura.', number: '24a', order: 67)
niif.contents.create!(text:'Una entidad revelará, en forma de tabla, los importes siguientes relacionados con las partidas cubiertas por separado por categoría de riesgo para los tipos de cobertura de la forma siguiente:
...
(iii) los saldos que permanecen en la reserva de cobertura de flujos de efectivo y la reserva de conversión de moneda extranjera de las relaciones de cobertura para las que deja de aplicarse la contabilidad de coberturas.', number: '24b', order: 68)
niif.contents.create!(text:'Una entidad revelará, en forma de tabla, los importes siguientes por separado por categoría de riesgo para los tipos de cobertura de la forma siguiente:
(a) para coberturas del valor razonable:
...
(vi) para coberturas de posiciones netas, las ganancias o pérdidas de cobertura reconocidas en partidas separadas en el estado del resultado integral (véase el párrafo 6.6.4 de la NIIF 9).', number: '24c', order: 69)
niif.contents.create!(text:'Cuando el volumen de las relaciones de cobertura a las cuales se aplica la exención del párrafo 23C no sea representativo de los volúmenes normales durante el periodo (es decir, el volumen en la fecha de presentación no refleja los volúmenes durante el periodo) una entidad revelará ese hecho y la razón por la que considera que los volúmenes no son representativos.', number: '24d', order: 70)
niif.contents.create!(text:'Una entidad proporcionará una conciliación de cada componente del patrimonio y un desglose de otro resultado integral de acuerdo con la NIC 1 que, tomados juntos:
...
(c) en el momento de la discontinuación de la medición de un instrumento de cobertura, o una proporción de éste, al valor razonable con cambios en resultados, ese valor razonable del instrumento financiero que ha pasado a ser el importe en libros nuevo de acuerdo con el párrafo 6.7.4 de la NIIF 9 y el importe principal o nominal relacionado (excepto para proporcionar información comparativa de acuerdo con la NIC 1, una entidad no necesita continuar revelando esta información en periodos posteriores).', number: '24g', order: 73)

niif.contents.create!(text: 'Valor Razonable', label: 3, order: 74)
niif.contents.create!(text:'Salvo por lo establecido en el párrafo 29, una entidad revelará el valor razonable correspondiente a cada clase de activos financieros y de pasivos financieros (véase el párrafo 6), de una forma que permita la realización de comparaciones con los correspondientes importes en libros.', number: '25', order: 75)
niif.contents.create!(text:'Al revelar los valores razonables, una entidad agrupará los activos financieros y los pasivos financieros en clases, pero sólo los compensará en la medida en que sus importes en libros estén compensados en el estado de situación financiera.', number: '26', order: 76)
niif.contents.create!(text:'[Eliminado]', number: '27', order: 77)
niif.contents.create!(text:'[Eliminado]', number: '27b', order: 78)
niif.contents.create!(text:'En algunos casos, una entidad no reconocerá una ganancia o pérdida en el reconocimiento inicial de un activo financiero o pasivo financiero porque el valor razonable ni se pone de manifiesto mediante un precio cotizado en un mercado activo para un activo o pasivo idéntico (es decir, un dato de entrada de Nivel 1) ni se basa en una técnica de valoración que utiliza solo datos procedentes de mercados observables (véase el párrafo B5.1.2A de la NIIF 9). En estos casos, la entidad revelará por clase de activo financiero o pasivo financiero:
...
(c) La razón por la cual la entidad concluyó que el precio de la transacción no era la mejor evidencia del valor razonable, incluyendo una descripción de la evidencia en que se basa el valor razonable.', number: '28', order: 79)
niif.contents.create!(text:'No se requiere información a revelar sobre el valor razonable:
...
(b) [eliminado]
(c) [eliminado]
(d) para pasivos por arrendamiento.', number: '29', order: 80)
niif.contents.create!(text:'[Eliminado]', number: '30', order: 81)

niif.contents.create!(text: 'Naturaleza Y Alcance De Los Riesgos Que Surgen De Los Instrumentos Financieros', label: 3, order: 82)
niif.contents.create!(text:'Una entidad revelará información que permita que los usuarios de sus estados financieros evalúen la naturaleza y el alcance de los riesgos que surgen de los instrumentos financieros a los que la entidad esté expuesta al final del periodo sobre el que se informa.', number: '31', order: 83)
niif.contents.create!(text:'Las informaciones requeridas por los párrafos 33 a 42 se centran en los riesgos procedentes de instrumentos financieros y en la manera en que se los gestiona. Dichos riesgos incluyen por lo general, sin que la enumeración sea taxativa, el riesgo de crédito, el riesgo de liquidez y el riesgo de mercado.', number: '32', order: 84)
niif.contents.create!(text:'La provisión de información cualitativa en el contexto de información a revelar cuantitativa permite a los usuarios vincular revelaciones relacionadas y así formarse una imagen global de la naturaleza y alcance de los riesgos que surgen de los instrumentos financieros. La interacción entre información a revelar cualitativa y cuantitativa contribuirá a revelar información de la forma que mejor permita a los usuarios evaluar la exposición de una entidad a los riesgos.', number: '32a', order: 85)

niif.contents.create!(text: 'Información Cualitativa', label: 3, order: 86)
niif.contents.create!(text:'Para cada tipo de riesgo que surja de los instrumentos financieros, una entidad
revelará:
...
(c) cualesquiera cambios habidos en (a) o (b) desde el período precedente.', number: '33', order: 87)

niif.contents.create!(text: 'Información Cuantitativa', label: 3, order: 88)
niif.contents.create!(text:'Para cada tipo de riesgo que surja de los instrumentos financieros, una entidad revelará:
...
(c) Las concentraciones de riesgo, si no resultan aparentes de la información a revelar realizada de acuerdo con los apartados (a) y (b).', number: '34', order: 89)
niif.contents.create!(text:'Si los datos cuantitativos revelados al final del periodo sobre el que se informa
fueran poco representativos de la exposición al riesgo de la entidad durante el período, una entidad suministrará información adicional que sea representativa.', number: '35', order: 90)

niif.contents.create!(text: 'Riesgo De Crédito', label: 3, order: 91)
niif.contents.create!(text: 'Alcance y objetivos', label: 5, order: 92)
niif.contents.create!(text:'Una entidad aplicará los requerimientos de revelar información de los párrafos 35F a 35N a los instrumentos financieros a los que se apliquen los requerimientos de deterioro de valor de la NIIF 9. Sin embargo:
...
(b) no se aplica el párrafo 35K(b) a las cuentas por cobrar por arrendamientos.', number: '35a', order: 93)
niif.contents.create!(text:'La información revelada sobre riesgo crediticio de acuerdo con los párrafos 35F a 35N permitirá a los usuarios de los estados financieros comprender el efecto del riesgo crediticio sobre el importe, calendario e incertidumbre de los flujos de efectivo futuros. Para lograr este objetivo, la información revelada sobre riesgo crediticio proporcionará:
...
(c) información sobre la exposición al riesgo crediticio (es decir, el riesgo crediticio inherente en los activos financieros de una entidad y compromisos para ampliar el crédito) incluyendo las concentraciones de riesgo crediticio significativas.', number: '35b', order: 94)
niif.contents.create!(text:' Una entidad no necesita duplicar información que ya se presente en otra parte, siempre que dicha información se incorpore mediante referencias cruzadas de los estados financieros con otros estados, como por ejemplo un comentario de la dirección o un informe sobre riesgos, que esté disponible para los usuarios de los estados financieros en las mismas condiciones y al mismo tiempo que éstos. Sin la información incorporada mediante referencias cruzadas, los estados financieros estarán incompletos.', number: '35c', order: 95)
niif.contents.create!(text:'Para cumplir los objetivos del párrafo 35B, una entidad considerará (excepto cuando se especifique otra cosa) cuánto detalle revelar, cuánto énfasis dar a los diferentes aspectos de los requerimientos de información a revelar, el nivel apropiado de agregación o desagregación y, si los usuarios de los estados financieros necesitan explicaciones adicionales para evaluar la información cuantitativa revelada.', number: '35d', order: 96)
niif.contents.create!(text:'Si la información a revelar proporcionada de acuerdo con los párrafos 35F a 35N, es insuficiente para alcanzar los objetivos del párrafo 35B, una entidad revelará la información adicional necesaria para cumplir con esos objetivos.', number: '35e', order: 97)

niif.contents.create!(text: 'Prácticas de gestión del riesgo crediticio', label: 3, order: 98)
niif.contents.create!(text:'Una entidad explicará sus prácticas de gestión del riesgo crediticio y cómo se relacionan con el reconocimiento y medición de las pérdidas crediticias esperadas. Para cumplir este objetivo una entidad revelará información que permita a los usuarios de los estados financieros comprender y evaluar:
...
(ii) controla la medida en que la corrección de valor por pérdidas de activos financieros que cumplen los criterios de (i) es posteriormente medida nuevamente por un importe igual a las pérdidas crediticias esperadas durante el tiempo de vida del activo, de acuerdo con el párrafo 5.5.3 de la NIIF 9.', number: '35f', order: 99)
niif.contents.create!(text:'Una entidad explicará los datos de entrada, supuestos y técnicas de estimación utilizadas para aplicar los requerimientos de la Sección 5.5 de la NIIF 9. Para este propósito, una entidad revelará:
...
(c) los cambios en las técnicas de estimación o supuestos significativos realizados durante el periodo de presentación y las razones de esos cambios.', number: '35g', order: 100)

niif.contents.create!(text: 'Información cuantitativa y cualitativa sobre los importes que surgen de las pérdidas crediticias esperadas', label: 3, order: 101)
niif.contents.create!(text:'Para explicar los cambios en las correcciones de valor por pérdidas y las razones para dichos cambios, una entidad proporcionará, por clase de instrumento financiero, una conciliación entre el saldo inicial y el final de la corrección de valor por pérdidas, en una tabla, mostrando por separado los cambios durante el periodo para:
...
(b) el importe en libros bruto al final del periodo de presentación de los activos financieros que han sido modificado desde el reconocimiento inicial en un momento en el que la corrección de valor por pérdidas se midió por un importe igual a las pérdidas crediticias esperadas durante el tiempo de vida del activo y para los que la corrección de valor por pérdidas ha cambiado durante el periodo de presentación a un importe igual a las pérdidas crediticias esperadas para 12 meses.', number: '35j', order: 104)
niif.contents.create!(text:'Para permitir a los usuarios de los estados financieros comprender el efecto de la garantía colateral y otras mejoras crediticias sobre los importes que surgen de las pérdidas crediticias esperadas, una entidad revelará por clase de instrumento financiero:
...
(ii) activos financieros que tienen deterioro crediticio en la fecha de presentación (pero que no tienen deterioro crediticio comprado u originado); y
(iii) cuentas por cobrar comerciales, activos del contrato o cuentas por cobrar por arrendamientos para los que las correcciones de valor por pérdidas se miden de acuerdo con el párrafo 5.5.15 de la NIIF 9.
(c) Que son activos financieros con deterioro de valor crediticio comprados u originados.', number: '35m', order: 108)
niif.contents.create!(text:'Para cuentas por cobrar comerciales, activos del contrato y cuentas por cobrar por arrendamientos a los que una entidad aplica el párrafo 5.5.15 de la NIIF 9, la información proporcionada de acuerdo con el párrafo 35M puede basarse en una matriz de provisiones (véase el párrafo B5.5.35 de la NIIF 9).', number: '35n', order: 109)
niif.contents.create!(text:'Para todos los instrumentos financieros dentro del alcance de esta NIIF, pero a los que no se aplican los requerimientos de deterioro de valor de la NIIF 9, una entidad revelará por clase de instrumento financiero:
...
(c) [eliminado]
(d) [eliminado]', number: '36', order: 110)
niif.contents.create!(text:'[Eliminado]', number: '37', order: 111)

niif.contents.create!(text: 'Garantías colaterales y otras mejoras crediticias obtenidas', label: 3, order: 112)
niif.contents.create!(text:'Cuando una entidad obtenga, durante el período, activos financieros o no financieros mediante la toma de posesión de garantías colaterales para asegurar el cobro, o ejecute otras mejoras crediticias (por ejemplo, avales), y tales activos cumplan los criterios de reconocimiento contenidos en otras NIIF, una entidad revelará sobre estos activos poseídos en la fecha sobre la que se informa:
(a) la naturaleza e importe en libros de los activos obtenidos; y
(b) cuando los activos no sean fácilmente convertibles en efectivo, sus políticas para disponer de tales activos, o para utilizarlos en sus operaciones.', number: '38', order: 113)

niif.contents.create!(text: 'Riesgo De Liquidez', label: 3, order: 115)
niif.contents.create!(text:'Una entidad revelará:
...
(c) Una descripción de cómo gestiona el riesgo de liquidez inherente en (a) y (b).', number: '39', order: 116)

niif.contents.create!(text: 'Riesgo De Mercado', label: 3, order: 117)
niif.contents.create!(text: 'Análisis de sensibilidad', label: 4, order: 118)
niif.contents.create!(text:'Salvo que una entidad cumpla lo establecido en el párrafo 41, revelará:
...
sensibilidad; y
(c) los cambios habidos desde el período anterior en los métodos e hipótesis utilizados, así como las razones de tales cambios.', number: '40', order: 119)
niif.contents.create!(text:'Si una entidad elaborase un análisis de sensibilidad, tal como el del valor en riesgo, que reflejase las interdependencias entre las variables de riesgo (por ejemplo, entre las tasas de interés y de cambio) y lo utilizase para gestionar riesgos financieros, podrá utilizar ese análisis de sensibilidad en lugar del especificado en el párrafo 40. La entidad revelará también:
...
(b) una explicación del objetivo del método utilizado, así como de las limitaciones que pudieran hacer que la información no reflejase plenamente el valor razonable de los activos y pasivos implicados.', number: '41', order: 120)

niif.contents.create!(text: 'Otra información a revelar sobre el riesgo de mercado', label: 3, order: 121)
niif.contents.create!(text:' Cuando los análisis de sensibilidad, revelados de acuerdo con los párrafos 40 o 41 no fuesen representativos del riesgo inherente a un instrumento financiero, por ejemplo, porque la exposición al final de año no refleja la exposición mantenida durante el mismo), la entidad revelará este hecho, así como la razón por la que cree que los análisis de sensibilidad carecen de representatividad.', number: '42', order: 122)

niif.contents.create!(text: 'Transferencias De Activos Financieros', label: 3, order: 123)
niif.contents.create!(text:' Los requerimientos de información a revelar de los párrafos 42B a 42H relativos a transferencias de activos financieros complementan los otros requerimientos a este respecto de esta NIIF. Una entidad presentará la información a revelar requerida en los párrafos 42B a 42H en una nota única en sus estados financieros. Una entidad proporcionará la información a revelar requerida para todos los activos financieros transferidos que no se den de baja en cuentas y para toda implicación continuada en un activo transferido, que exista en la fecha de presentación, independientemente del momento en que tenga lugar la transacción de transferencia relacionada. A efectos de la aplicación de los requerimientos de información a revelar de esos párrafos, una entidad transfiere total o parcialmente un activo financiero (el activo financiero transferido) si y solo si se da, al menos, una de las siguientes condiciones:
(a) transfiere los derechos contractuales a recibir los flujos de efectivo de ese activo financiero; o
(b) retiene los derechos contractuales a recibir los flujos de efectivo de ese activo financiero, pero asume en un acuerdo, una obligación contractual de pagar dichos flujos de efectivo a uno o más receptores.', number: '42a', order: 124)
niif.contents.create!(text:'Una entidad revelará información que permita a los usuarios de sus estados financieros:
...
(c) un acuerdo mediante el cual una entidad retiene los derechos contractuales a recibir los flujos de efectivo de un activo financiero, pero asume una obligación contractual de pagar los flujos de efectivo a una o más entidades y se cumplen las condiciones del párrafo 3.2.5(a) a (c) de la NIIF 9.', number: '42c', order: 126)

niif.contents.create!(text: 'Activos Financieros Transferidos Que No Se Dan De Baja En Cuentas En Su Totalidad', label: 3, order: 127)
niif.contents.create!(text:'Una entidad puede haber transferido activos financieros de tal forma que los activos financieros transferidos total o parcialmente no cumplen los requisitos para su baja en cuentas. Para cumplir los objetivos establecidos en el párrafo 42B(a), la entidad revelará en cada fecha de presentación para cada clase de activos financieros transferidos que no se dan de baja en su totalidad:
...
(f) Información cualitativa que explique y dé soporte a la información cuantitativa requerida en (a) a (e).', number: '42e', order: 130)
niif.contents.create!(text:' Una entidad puede agregar la información requerida por el párrafo 42E con respecto a un activo en particular si ésta tiene más de un tipo de implicación continuada en ese activo financiero dado de baja en cuentas, e informar sobre el mismo según un tipo de implicación continuada.', number: '42f', order: 131)
niif.contents.create!(text:'Además, una entidad revelará para cada tipo de implicación continuada:
...
Una entidad proporcionará esta información para cada periodo para el cual se presente un estado del resultado integral.', number: '42g', order: 132)
